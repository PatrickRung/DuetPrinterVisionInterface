import { RawMapEntityType, RawMapLayerType } from "./api";
...
class StructureManager {
    ...
    updateMapStructuresFromMapData(rawMap) {
        this.mapStructures = [
    ...
    buildMapStructuresFromEntityMapData(entities) {
        const mapStructures = [];
        entities.forEach(e => {
            switch (e.type) {
                ...
                case RawMapEntityType.ActiveZone: {
                    ...
                    mapStructures.push(new ActiveZoneMapStructure(p0.x, p0.y, p1.x, p1.y, p2.x, p2.y, p3.x, p3.y));
                    break;
                }
                case RawMapEntityType.NoGoArea: {
                    ...
                    const p2 = this.convertCMCoordinatesToPixelSpace({ x: e.points[4], y: e.points[5] });
                    const p3 = this.convertCMCoordinatesToPixelSpace({ x: e.points[6], y: e.points[7] });
                    mapStructures.push(new NoGoAreaMapStructure(p0.x, p0.y, p1.x, p1.y, p2.x, p2.y, p3.x, p3.y));
                    break;
                }
                case RawMapEntityType.NoMopArea: {
                    ...
                    const p2 = this.convertCMCoordinatesToPixelSpace({ x: e.points[4], y: e.points[5] });
                    const p3 = this.convertCMCoordinatesToPixelSpace({ x: e.points[6], y: e.points[7] });
                    mapStructures.push(new NoMopAreaMapStructure(p0.x, p0.y, p1.x, p1.y, p2.x, p2.y, p3.x, p3.y));
                    break;
                }
                case RawMapEntityType.VirtualWall: {
                    const p0 = this.convertCMCoordinatesToPixelSpace({ x: e.points[0], y: e.points[1] });
                    const p1 = this.convertCMCoordinatesToPixelSpace({ x: e.points[2], y: e.points[3] });
                    mapStructures.push(new VirtualWallMapStructure(p0.x, p0.y, p1.x, p1.y));
                    break;
                }
            }
        });
    ...
    buildMapStructuresFromLayerMapData(layers) {
        const mapStructures = [];
        const previouslySelectedSegmentLabelsMap = {};
        ...
        layers.forEach(l => {
            switch (l.type) {
                case RawMapLayerType.Segment: {
                     ...
                     */
                    if (Math.abs(l.dimensions.x.mid - l.dimensions.x.avg) >= 8 ||
                    ...
                    mapStructures.push(new SegmentLabelMapStructure(coords.x, coords.y, l.metaData.segmentId ?? "", !!previouslySelectedSegmentLabelsMap[l.metaData.segmentId ?? ""], !!l.metaData.active, l.metaData.area, l.metaData.name));
                    break;
                }
            }
        });
    ...
    addClientStructure(structure) {
        this.clientStructures.push(structure);
    }
    removeClientStructure(structure) {
...
const TYPE_SORT_MAPPING = {
    ...
    [TestingGridClientStructure.TYPE]: 5,
    [PrintObjectMapStructure.TYPE]: 6,
    [PrintObjectClientStructure.TYPE]: 8,
    [TestPointLabelClientStructure.TYPE]: 9,
    [TestPointClientStructure.TYPE]: 5,
