...
export function convertSecondsToHumans(seconds, showSeconds = true, showDays = true) {
    let levels;
    if (showDays) {
        levels = [
            {
...
export function convertBytesToHumans(bytes) {
...
const consumableSubtypeMapping = {
    "main": "Main",
    "secondary": "Secondary",
    "side_right": "Right",
    "side_left": "Left",
...
export const getConsumableName = (type, subType) => {
    let ret = "";
...
export function getFriendlyStatName(stat) {
    switch (stat.type) {
        case "area":
            return "Area";
        case "time":
...
export function getHumanReadableStatValue(stat) {
    switch (stat.type) {
        case "area":
...
export function adjustColorBrightness(hexInput, percent) {
    let hex = hexInput;
