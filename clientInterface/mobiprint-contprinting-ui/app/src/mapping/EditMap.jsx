...
class EditMap extends Map {
    ...
    updateVirtualRestrictionClientStructures(remove) {
        if (remove) {
            ...
            this.props.rawMap.entities.forEach(e => {
                switch (e.type) {
                    case RawMapEntityType.VirtualWall: {
                        ...
                        this.structureManager.addClientStructure(new VirtualWallClientStructure(p0.x, p0.y, p1.x, p1.y, false));
                        break;
                    }
                    case RawMapEntityType.NoGoArea: {
                        ...
                        this.structureManager.addClientStructure(new NoGoAreaClientStructure(p0.x, p0.y, p1.x, p1.y, p2.x, p2.y, p3.x, p3.y, false));
                        break;
                    }
                    case RawMapEntityType.NoMopArea: {
                        ...
                        const p2 = this.structureManager.convertCMCoordinatesToPixelSpace({ x: e.points[4], y: e.points[5] });
                        const p3 = this.structureManager.convertCMCoordinatesToPixelSpace({ x: e.points[6], y: e.points[7] });
                        this.structureManager.addClientStructure(new NoMopAreaClientStructure(p0.x, p0.y, p1.x, p1.y, p2.x, p2.y, p3.x, p3.y, false));
                        break;
                    }
                }
            });
        ...
        if (this.props.robotStatus.value === "docked") {
            ...
            if (this.props.mode === "segments" &&
                ...
                if (intersectingSegmentId) {
                    ...
                    if (this.state.selectedSegmentIds.length < 2 ||
    ...
    render() {
        return (<MapContainer style={{ overflow: "hidden" }}>
                ...
                <HelpAction helpDialogOpen={this.state.helpDialogOpen} setHelpDialogOpen={(open) => {
                ...
                <SegmentActions robotStatus={this.props.robotStatus} selectedSegmentIds={this.state.selectedSegmentIds} segmentNames={this.state.segmentNames} cuttingLine={this.state.cuttingLine} convertPixelCoordinatesToCMSpace={(coordinates => {
                        ...
                        this.structureManager.addClientStructure(new CuttingLineClientStructure(p0.x, p0.y, p1.x, p1.y, true));
                        this.updateState();
                        this.draw();
                    }} onClear={() => {
                ...
                <VirtualRestrictionActions robotStatus={this.props.robotStatus} virtualWalls={this.state.virtualWalls} noGoAreas={this.state.noGoAreas} noMopAreas={this.state.noMopAreas} convertPixelCoordinatesToCMSpace={(coordinates => {
                        return this.structureManager.convertPixelCoordinatesToCMSpace(coordinates);
                    })} onAddVirtualWall={() => {
                        const currentCenter = this.getCurrentViewportCenterCoordinatesInPixelSpace();
                        const p0 = {
                        ...
                        this.structureManager.addClientStructure(new VirtualWallClientStructure(p0.x, p0.y, p1.x, p1.y, true));
                        this.updateState();
                        this.draw();
                    }} onAddNoGoArea={() => {
                        const currentCenter = this.getCurrentViewportCenterCoordinatesInPixelSpace();
                        ...
                        this.structureManager.addClientStructure(new NoGoAreaClientStructure(p0.x, p0.y, p1.x, p1.y, p2.x, p2.y, p3.x, p3.y, true));
                        this.updateState();
                        this.draw();
                    }} onAddNoMopArea={() => {
                        const currentCenter = this.getCurrentViewportCenterCoordinatesInPixelSpace();
                        ...
                        this.structureManager.addClientStructure(new NoMopAreaClientStructure(p0.x, p0.y, p1.x, p1.y, p2.x, p2.y, p3.x, p3.y, true));
                        this.updateState();
                        this.draw();
                    }} onRefresh={() => {
                        this.updateVirtualRestrictionClientStructures(true);
                ...
                <HelpDialog dialogOpen={this.state.helpDialogOpen} setDialogOpen={(open) => {
                this.setState({ helpDialogOpen: open });
