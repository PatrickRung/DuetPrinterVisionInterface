...
class SegmentLabelMapStructure extends MapStructure {
    constructor(x0, y0, id, selected, active, area, name) {
        super(x0, y0);
        this.scaledIconSize = { width: 1, height: 1 };
        this.id = id;
    ...
    draw(ctxWrapper, transformationMatrixToScreenSpace, scaleFactor) {
        ...
        this.scaledIconSize = {
            width: Math.max(imageToUse.width * (scaleFactor / 4), imageToUse.width * 0.8),
            height: Math.max(imageToUse.height * (scaleFactor / 4), imageToUse.height * 0.8)
        };
        ctxWrapper.save();
        if (this.active) {
        ...
        // ctx.drawImage(imageToUse, p0.x - this.scaledIconSize.width / 2, p0.y - (this.scaledIconSize.height / 3) * 2, this.scaledIconSize.width, this.scaledIconSize.height);
        ctxWrapper.restore();
...
SegmentLabelMapStructure.TYPE = "SegmentLabelMapStructure";
