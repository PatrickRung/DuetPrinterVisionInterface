...
const SegmentActions = (props) => {
    const { selectedSegmentIds, segmentNames, cuttingLine, convertPixelCoordinatesToCMSpace, supportedCapabilities, onAddCuttingLine, onClear } = props;
    const [newSegmentName, setNewSegmentName] = React.useState("");
    const [renameDialogOpen, setRenameDialogOpen] = React.useState(false);
    const { mutate: joinSegments, isLoading: joinSegmentsExecuting } = useJoinSegmentsMutation({
        onSuccess: onClear,
    });
    const { mutate: splitSegment, isLoading: splitSegmentExecuting } = useSplitSegmentMutation({
        onSuccess: onClear,
    });
    const { mutate: renameSegment, isLoading: renameSegmentExecuting } = useRenameSegmentMutation({
        onSuccess: onClear,
    });
    ...
    return (<Grid container spacing={1} direction="row-reverse" flexWrap="wrap-reverse">
            ...
            <Grid item>
                    <ActionButton disabled={joinSegmentsExecuting || !canEdit} color="inherit" size="medium" variant="extended" onClick={handleJoinClick}>
                        Join {segmentNames[selectedSegmentIds[0]]} and {selectedSegmentIds.length === 2 ? segmentNames[selectedSegmentIds[1]] : "?"}
                        {joinSegmentsExecuting && (<CircularProgress color="inherit" size={18} style={{ marginLeft: 10 }}/>)}
                    </ActionButton>
                </Grid>}
            {supportedCapabilities[Capability.MapSegmentEdit] &&
            ...
            cuttingLine !== undefined &&
            <Grid item>
                    <ActionButton disabled={splitSegmentExecuting || !canEdit} color="inherit" size="medium" variant="extended" onClick={handleSplitClick}>
                        Split {segmentNames[selectedSegmentIds[0]]}
                        {splitSegmentExecuting && (<CircularProgress color="inherit" size={18} style={{ marginLeft: 10 }}/>)}
                    </ActionButton>
                </Grid>}
            ...
            cuttingLine === undefined &&
            <Grid item>
                    <ActionButton disabled={renameSegmentExecuting || !canEdit} color="inherit" size="medium" variant="extended" onClick={() => {
            ...
            cuttingLine === undefined &&
            <Grid item>
                    <ActionButton disabled={joinSegmentsExecuting || !canEdit} color="inherit" size="medium" variant="extended" onClick={onAddCuttingLine}>
                        Add Cutting Line
                    </ActionButton>
                </Grid>}
            ...
            <Dialog open={renameDialogOpen} onClose={() => {
                    ...
                    <DialogContent>
                        ...
                        <TextField autoFocus margin="dense" variant="standard" label="Segment name" fullWidth value={newSegmentName} onChange={(e) => {
