...
const VirtualRestrictionActions = (props) => {
    const { virtualWalls, noGoAreas, noMopAreas, convertPixelCoordinatesToCMSpace, onAddVirtualWall, onAddNoGoArea, onAddNoMopArea, onSave, onRefresh } = props;
    const { data: combinedVirtualRestrictionsProperties, isLoading: combinedVirtualRestrictionsPropertiesLoading, isError: combinedVirtualRestrictionsPropertiesLoadError, refetch: refetchCombinedVirtualRestrictionsProperties, } = useCombinedVirtualRestrictionsPropertiesQuery();
    const { mutate: saveRestrictions, isLoading: restrictionsSaving } = useCombinedVirtualRestrictionsMutation({
        onSuccess: onSave,
    });
    ...
    return (<Grid container spacing={1} direction="row-reverse" flexWrap="wrap-reverse">
            {canEdit &&
            <Grid item>
                    <ActionButton disabled={restrictionsSaving} color="inherit" size="medium" variant="extended" onClick={handleSaveClick}>
                        Save
                    </ActionButton>
                </Grid>}
            {canEdit &&
            <Grid item>
                    <ActionButton color="inherit" size="medium" variant="extended" onClick={onAddVirtualWall}>
                        Add Virtual Wall
                    </ActionButton>
                </Grid>}
            {canEdit &&
            ...
            <Grid item>
                    <ActionButton color="inherit" size="medium" variant="extended" onClick={onAddNoGoArea}>
                        Add No Go Area
                    </ActionButton>
                </Grid>}
            {canEdit &&
            ...
            <Grid item>
                    <ActionButton color="inherit" size="medium" variant="extended" onClick={onAddNoMopArea}>
                        Add No Mop Area
                    </ActionButton>
                </Grid>}
            ...
            {canEdit &&
            <Grid item>
                    <ActionButton color="inherit" size="medium" variant="extended" onClick={onRefresh}>
                        Refresh
                    </ActionButton>
                </Grid>}
            {!canEdit &&
