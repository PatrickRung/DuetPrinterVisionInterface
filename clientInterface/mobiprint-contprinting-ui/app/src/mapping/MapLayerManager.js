import { adjustColorBrightness } from "./utils/utils";
...
export class MapLayerManager {
    constructor() {
        ...
        this.mapLayerManagerWorkerLastNonce = "";
        this.width = 1;
        this.height = 1;
        ...
        this.mapLayerManagerWorker.onmessage = (evt) => {
            if (evt.data.pixelData !== undefined) {
                const imageData = new ImageData(new Uint8ClampedArray(evt.data.pixelData), evt.data.width, evt.data.height);
    ...
    draw(data, theme) {
        ...
        return new Promise((resolve, reject) => {
            ...
            if (this.canvas.width !== Math.round(data.size.x / data.pixelSize) ||
            ...
            if (data.layers.length > 0) {
                if (this.mapLayerManagerWorkerAvailable) {
                    ...
                    const rendered = PROCESS_LAYERS(data.layers, data.pixelSize, colors, backgroundColors, this.selectedSegmentIds);
                    this.segmentLookupInfo = {
     ...
     */
    getIntersectingSegment(x, y) {
        if (x < this.segmentLookupInfo.left ||
        ...
        const offset = Math.round((Math.round(x) - this.segmentLookupInfo.left) +
